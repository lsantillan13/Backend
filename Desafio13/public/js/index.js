window.onload = function(){ // => When document is ready...
  ...
  const loginSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:8080/api/signup', {username}, config);
      window.localStorage.setItem('username', username);
      location.reload();}
      
    catch(err){ console.log(err);}
  };
  document.getElementById('login').addEventListener('click', loginSubmit);
  if (user){
    let username = window.localStorage.getItem('username');
    /*DOM*/
    let userForm = document.getElementById('userForm').style.display = 'none';
    let userStatus = document.getElementById('userStatus');
    let userInfo = document.getElementById('userInfo').textContent = `Bienvenido ${username}`;
    let logOut = document.createElement('button');
    /*Render*/
    userStatus.appendChild(logOut);
    logOut.innerHTML = 'Cerrar sesión';
    logOut.classList.add('logOut');
    /*LOGOUT*/
    logOut.addEventListener('click', async(e) => {
      e.preventDefault();
      let cyaLater  = document.getElementById('userInfo').textContent = `Hasta luego!`;
