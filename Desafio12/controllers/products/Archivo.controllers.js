...
class ContenedorArchivo{
  ...
  async createProduct(req, res){
    ...
    if (fs.existsSync(this.ruta)) {
      ...
      if (parsed.length === 0) {
        ...
        return fs.writeFile(this.ruta, defaultProduct, (err) =>
          err
            ? console.log(`Error: ${err}`)
            : console.log('{', this.ruta, '} File Created succesfuly.')
        ) &&
        ...
        fs.writeFileSync(this.ruta, JSON.stringify(parsed, 8, '\t'));
      ...
      const { title, price, img } = req.body;
      let defaultProduct = `[{  "title": "${title}",\n \t"price": "${price}",\n \t"img": "${img}",\n \t"id": ${1}}\n]`;
      fs.writeFile(this.ruta, defaultProduct, (err) =>
        err
          ? console.log(`Error: ${err}`)
          : console.log('{', this.ruta, '} File Created succesfuly.')
      );
  ...
  async getAll(req, res) {
    ...
    if (fs.existsSync(this.ruta) === true) {
      const data = fs.readFileSync(this.ruta, 'utf-8');
  ...
  updateById(req, res, next) {
    try {
      ...
      if (A[0].id == productId) {
        ...
        fs.writeFileSync(this.ruta, JSON.stringify(sorted, 8, '\t'));
        res.status(200).send();
  ...
  deleteById(req, res, next) {
    try {
      ...
      if (B[0].id == productId) {
        fs.writeFileSync(this.ruta, STRING);
