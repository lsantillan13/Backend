...
save(req, res){
    ...
    /*Si el documento Existe...*/
     if(fs.existsSync(DIR)){
        ...
        /*Si tiene longitud diferente a 0, crea el proximo producto con su longitud +1 */
            ...
            ARR.push({title, price, img, id: ARR.length + 1})
    ...
    /*En caso que el documento no exista, lo crea y le otorga al producto el id 1*/
        ...
        fs.writeFile(DIR, defaultProduct, err => err ? console.log(`Error: ${err}`) : console.log('{', DIR, '} File Created succesfuly.'))
        ARR.push({title, price, img, id: 1})
        res.status(200).send({Message: `File Created & Product added under ID: 1`})    
    }
...
async getAll(req, res){
    ...
    if(fs.existsSync(DIR) === true){ const data = fs.readFileSync(DIR, 'utf-8'); await res.send(data)}
    /*Si no existe lo crea y muestra un array vacío*/
    else{ fs.writeFileSync(DIR, '[]');  const data = fs.readFileSync(DIR, 'utf-8'); await res.send(data)}
}
...
getById(req, res, next){
    ...
    /*Filtro el id*/
        let A = parsed.filter(x => {return x.id == productId})
...
updateById(req, res, next){
    try{
        ...
        const {title, price, img,} = req.body;
        /*Finded*/
        let A = PARSED.filter(e => {return e.id == productId})
        /*Without*/
        let B = PARSED.filter(e => {return e.id != productId})
        /*NewArray*/
        let C = [];
        ...
        if(A[0].id == productId){
            ...
            const sorted = C[0].sort(function(a, b) { return a.id-b.id});
